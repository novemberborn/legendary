...
ResolutionPropagator.prototype.resolve = function(rejected, result,
    handledRejection) {
  if (this._resolved) {
  ...
  if (state === REJECTED) {
    this._handledRejection = handledRejection ||
        legendary.unhandledRejection(result);
  } else if (state === PENDING) {
...
ResolutionPropagator.prototype._then = function(onFulfilled, onRejected) {
  ...
  var propagator = new ResolutionPropagator(this._constructor,
...
ResolutionPropagator.prototype._resolveThenable = function() {
  var promise = adoptThenable(this._constructor, this._result,
      this._resultThen);
