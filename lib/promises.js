...
Promise.from = function(value){
  return new ResolutionPropagator(this, null, true).resolve(false, value).promise();
};
...
function NeverCancellable(resolver){
  if(typeof resolver !== "function"){
    throw new TypeError();
  }
  ...
  if(resolver !== blessed.be){
    blessed.be(this, resolver, false);
  }
}

exports.NeverCancellable = NeverCancellable;

NeverCancellable.prototype = new Promise(blessed.be);
NeverCancellable.prototype.constructor = NeverCancellable;

NeverCancellable.from = function(value){
  return new ResolutionPropagator(this, null, false).resolve(false, value).promise();
};

NeverCancellable.rejected = function(reason){
  return new ResolutionPropagator(this, null, false).resolve(
    true, reason, legendary.unhandledRejection(reason)
  ).promise();
};
