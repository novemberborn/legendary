...
function trampoline(queue){
  ...
  while(stackIndex){
    ...
    while(queueIndex < queueLength && topOfStack){
      queue[queueIndex].resolve(queue[queueIndex + 1], queue[queueIndex + 2], queue[queueIndex + 3]);
...
// if already running.
exports.sameTurn = function(propagator, rejected, result, handledRejection){
  trampoline([propagator, rejected, result, handledRejection]);
};
