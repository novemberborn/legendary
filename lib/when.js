...
//
// * If a pending promise was received, the result of passing the `onFulfilled`
//   callback to its then() method is returned.
// * If a rejected promise was received, and no `onRejected` callback is
//   passed, a promise adopting its state is returned instead.
// * If a fulfilled promise was received, its fulfillment value is passed to
//   the `onFulfilled` callback. If the callback throws, a rejected promise is
//   returned with the thrown exception as rejection reason. If the callback
//   returns a non-fulfilled promise or value, this result is returned as-is.
//   If the callback returns a fulfilled promise, its fulfillment value is
//   returned instead. If the callback returns a thenable, a promise adopting
//   its state is returned instead.
...
//
// * If a pending promise was received, the result of passing the `onRejected`
//   callback to its then() method is returned.
// * If a fulfilled promise was received, and no `onFulfilled` callback is
//   passed, its fulfillment value is returned.
// * If a rejected promise was received, its reason is passed to the
//   `onRejected` callback. If the callback throws, a rejected promise is
//   returned with the thrown exception as rejection reason. If the callback
//   returns a non-fulfilled promise or value, this result is returned as-is.
//   If the callback returns a fulfilled promise, its fulfillment value is
//   returned instead. If the callback returns a thenable, a promise adopting
...
function when(value, onFulfilled, onRejected){
  if(value instanceof promises.Promise && arguments.length === 1){
    return value;
  }
