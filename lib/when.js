...
//   error as rejection reason. If the callback returns a promise or value,
...
function when(value, onFulfilled, onRejected){
  ...
  var propagator = new ResolutionPropagator().settle(true, value);

  if(arguments.length <= 1){
    return propagator.promise;
  }

  if(propagator.hasThenable()){
  ...
  try{
    return ResolutionPropagator.valueOrPromise(onFulfilled(propagator.value));
  }catch(error){
    return new ResolutionPropagator().settle(false, error).promise;
  }
}
module.exports = when;
